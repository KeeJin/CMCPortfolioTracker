...
const RAW_BUY_AMZN =
  "18/11/2025 33784 Bght 10 AMZN:US @ 301.5441 SGD 3,015.44 120,254.56";
const RAW_SELL_TXRH =
  "18/11/2025 33797 Sold 15 TXRH:US @ 220.9747 SGD 3,314.62 123,569.18";
const RAW_DIVIDEND_ASML =
  "10/11/2025 3959750 JNL3959750 ASML:US Intl Div Ex:29/10/25 8.26 99,998.61";
const RAW_DEPOSIT =
  "10/11/2025 21788176 Dep CHASSGSG 100000003011513 100000.00 99,990.35";
const RAW_WITHDRAWAL =
  "18/11/2025 30607526 Wdl CHASSGSG 100000003011513 33784(FX) 5000.00 118,569.18";
const RAW_BUY_HOOD =
  "18/11/2025 33799 Bght 22 HOOD:US @ 154.2137 SGD 3,392.70 116,861.86";
const RAW_BUY_META =
  "24/11/2025 34151 Bght 6 META:US @ 786.2644 SGD 4,717.59 112,144.27";

// ─── classifyTransactionType ──────────────────────────────────────────────────

describe("classifyTransactionType", () => {
  it("classifies BUY", () => {
    expect(classifyTransactionType("Bght 10 AMZN:US @ 301.5441")).toBe("BUY");
  });
  it("classifies SELL", () => {
    expect(classifyTransactionType("Sold 15 TXRH:US @ 220.9747")).toBe("SELL");
  });
  it("classifies DIVIDEND", () => {
    expect(classifyTransactionType("JNL3959750 ASML:US Intl Div Ex:29/10/25")).toBe("DIVIDEND");
  });
  it("classifies DEPOSIT", () => {
    expect(classifyTransactionType("Dep CHASSGSG 100000003011513")).toBe("DEPOSIT");
  });
  it("classifies WITHDRAWAL", () => {
    expect(classifyTransactionType("Wdl CHASSGSG 100000003011513")).toBe("WITHDRAWAL");
  });
  it("returns UNKNOWN for unrecognised text", () => {
    expect(classifyTransactionType("Some random text")).toBe("UNKNOWN");
  });
});
...
describe("extractSymbol", () => {
  it("extracts AMZN from buy description", () => {
    expect(extractSymbol("Bght 10 AMZN:US @ 301.5441")).toBe("AMZN");
  });
  it("extracts TXRH from sell description", () => {
    expect(extractSymbol("Sold 15 TXRH:US @ 220.9747")).toBe("TXRH");
  });
  it("extracts ASML from dividend description", () => {
    expect(extractSymbol("JNL3959750 ASML:US Intl Div Ex:29/10/25")).toBe("ASML");
  });
  it("returns null for deposit (no :US symbol)", () => {
    expect(extractSymbol("Dep CHASSGSG 100000003011513")).toBeNull();
  });
  it("returns null for withdrawal", () => {
    expect(extractSymbol("Wdl CHASSGSG 100000003011513")).toBeNull();
  });
});
...
describe("extractQuantity", () => {
  it("extracts quantity 10 from AMZN buy", () => {
    expect(extractQuantity("Bght 10 AMZN:US @ 301.5441")).toBe(10);
  });
  it("extracts quantity 22 from HOOD buy", () => {
    expect(extractQuantity("Bght 22 HOOD:US @ 154.2137")).toBe(22);
  });
  it("extracts quantity 15 from TXRH sell", () => {
    expect(extractQuantity("Sold 15 TXRH:US @ 220.9747")).toBe(15);
  });
  it("returns null for dividend (no Bght/Sold)", () => {
    expect(extractQuantity("JNL3959750 ASML:US Intl Div Ex:29/10/25")).toBeNull();
  });
});
...
describe("extractPrice", () => {
  it("extracts price 301.5441 from AMZN buy", () => {
    expect(extractPrice("Bght 10 AMZN:US @ 301.5441")).toBe(301.5441);
  });
  it("extracts price 220.9747 from TXRH sell", () => {
    expect(extractPrice("Sold 15 TXRH:US @ 220.9747")).toBe(220.9747);
  });
...
describe("calculateAmount", () => {
  it("returns negative value for debit-only (outflow)", () => {
    expect(calculateAmount(3015.44, null)).toBe(-3015.44);
  });
  it("returns positive value for credit-only (inflow)", () => {
    expect(calculateAmount(null, 8.26)).toBe(8.26);
  });
  it("credit takes precedence over debit", () => {
    expect(calculateAmount(100, 200)).toBe(200);
  });
  it("returns null when both are null", () => {
    expect(calculateAmount(null, null)).toBeNull();
  });
});
...
describe("parseRawRow", () => {
  it("parses a BUY row correctly", () => {
    const result = parseRawRow(RAW_BUY_AMZN);
  ...
  it("parses a SELL row correctly", () => {
    const result = parseRawRow(RAW_SELL_TXRH);
    expect(result).not.toBeNull();
    expect(result!.date).toBe("18/11/2025");
  ...
  it("parses a DIVIDEND row correctly", () => {
    const result = parseRawRow(RAW_DIVIDEND_ASML);
    expect(result).not.toBeNull();
  ...
  it("returns null for empty string", () => {
    expect(parseRawRow("")).toBeNull();
  });

  it("returns null for text with no date prefix", () => {
    expect(parseRawRow("no date here")).toBeNull();
  });
});
...
describe("parseTransactionRow — BUY", () => {
  it("parses AMZN buy from raw text", () => {
    const result = parseTransactionRow(RAW_BUY_AMZN, "statement.pdf");
  ...
  it("parses HOOD buy from raw text", () => {
    const result = parseTransactionRow(RAW_BUY_HOOD, "statement.pdf");
    expect(result.ok).toBe(true);
    if (!result.ok) return;
    expect(result.transaction.type).toBe("BUY");
  ...
  it("parses META buy from raw text", () => {
    const result = parseTransactionRow(RAW_BUY_META, "statement.pdf");
    expect(result.ok).toBe(true);
    if (!result.ok) return;
    expect(result.transaction.type).toBe("BUY");
...
describe("parseTransactionRow — SELL", () => {
  it("parses TXRH sell from raw text", () => {
    const result = parseTransactionRow(RAW_SELL_TXRH, "statement.pdf");
    expect(result.ok).toBe(true);
    if (!result.ok) return;
...
describe("parseTransactionRow — DIVIDEND", () => {
  it("parses ASML dividend from raw text", () => {
    const result = parseTransactionRow(RAW_DIVIDEND_ASML, "statement.pdf");
    expect(result.ok).toBe(true);
    if (!result.ok) return;
...
describe("parseTransactionRow — DEPOSIT", () => {
  it("parses deposit from raw text", () => {
    const result = parseTransactionRow(RAW_DEPOSIT, "statement.pdf");
    expect(result.ok).toBe(true);
    if (!result.ok) return;
...
describe("parseTransactionRow — WITHDRAWAL", () => {
  it("parses withdrawal from raw text", () => {
    const result = parseTransactionRow(RAW_WITHDRAWAL, "statement.pdf");
    expect(result.ok).toBe(true);
    if (!result.ok) return;
...
describe("parseTransactionRow — error cases", () => {
  it("returns error for completely unparseable text", () => {
    const result = parseTransactionRow("not a valid row");
