...
const baseline_1 = require("../../src/parsers/baseline");
...
describe("extractBaselineDate", () => {
    it("extracts and converts date from header line", () => {
        expect((0, baseline_1.extractBaselineDate)(SAMPLE_HEADER)).toBe("2026-01-01");
    });
    it("returns null when date line is absent", () => {
        expect((0, baseline_1.extractBaselineDate)("No date here")).toBeNull();
    });
    it("handles different dates correctly", () => {
        expect((0, baseline_1.extractBaselineDate)("At Close of Trading Day: 28/04/2026")).toBe("2026-04-28");
    });
});
// ─── extractCash ─────────────────────────────────────────────────────────────
describe("extractCash", () => {
    it("extracts bank balance from summary section", () => {
        expect((0, baseline_1.extractCash)(SAMPLE_SUMMARY)).toBe(5890.15);
    });
    it("returns null when Bank Balance line is absent", () => {
        expect((0, baseline_1.extractCash)("No bank balance here")).toBeNull();
    });
    it("handles balances with no comma separator", () => {
        expect((0, baseline_1.extractCash)("Bank Balance 100.00")).toBe(100.0);
    });
    it("handles large balances with commas", () => {
        expect((0, baseline_1.extractCash)("Bank Balance 1,234,567.89")).toBe(1234567.89);
    });
});
// ─── extractHoldings ─────────────────────────────────────────────────────────
describe("extractHoldings", () => {
    it("extracts all holdings from layout-format text", () => {
        const holdings = (0, baseline_1.extractHoldings)(SAMPLE_HOLDINGS_ROWS);
...
describe("parseBaselineText — success", () => {
    ...
    it("returns correct holdings counts", () => {
        const result = (0, baseline_1.parseBaselineText)(SAMPLE_TEXT, "baseline-001");
        expect(result.ok).toBe(true);
        if (!result.ok)
            return;
...
describe("parseBaselineText — error cases", () => {
    it("returns error when date is missing", () => {
        ...
        const result = (0, baseline_1.parseBaselineText)(noDate, "x");
    ...
    it("returns error when Bank Balance is missing", () => {
        ...
        const result = (0, baseline_1.parseBaselineText)(noCash, "x");
        expect(result.ok).toBe(false);
        if (result.ok)
            return;
    ...
    it("returns error when no holdings are found", () => {
        ...
        const result = (0, baseline_1.parseBaselineText)(noHoldings, "x");
        expect(result.ok).toBe(false);
        if (result.ok)
            return;
